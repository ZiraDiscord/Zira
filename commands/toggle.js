...
exports.Run = async function Run(caller, command, guild, lang) {
  ...
  if (process.env.PREMIUM && !guild.premium && guild.roles.filter((r) => r.toggle).length > 5) {
    caller.utils.createMessage(command.msg.channel.id, {
      embed: {
        color: caller.color.yellow,
        title: lang.titles.error,
  ...
  if (process.env.PREMIUM && !guild.premium && guild.roles.filter((r) => r.toggle).length + params.length > 6) {
    caller.utils.createMessage(command.msg.channel.id, {
      embed: {
        color: caller.color.yellow,
        title: lang.titles.error,
  ...
  for (let index = 0; index < params.length; index++) {
    ...
    try {
      ...
      caller.logger.warn(`[Toggle] ${command.msg.channel.id} ${e.code} ${e.message.replace(/\n\s/g, '')}`);
      switch (e.code) {
        case 10003:
