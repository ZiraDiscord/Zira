...
class Zira {
  constructor({ firstShardID, lastShardID, maxShards, cluster }) {
      ...
      if (fs.existsSync(`./commands/${command.command}.js`)) {
        try {
          ...
          if (command.msg.channel.guild) {
            ...
            guild.icon = command.msg.channel.guild.iconURL
              ? command.msg.channel.guild.iconURL.replace('.jpg', '.png')
              : '';
          }
    ...
    this.bot.on('shardDisconnect', (message, shard) => {
      this.logger.warn(`[Shard] ${shard} disconnected ${JSON.stringify(message)}`);
    });
