...
exports.Run = async function Run(caller, command, guild, lang) {
  ...
  for (let index = 0; index < params.length; index++) {
    const [role] = command.guild.roles.filter((r) => r.id === params[index] || r.name.toLowerCase() === params[index].toLowerCase());
    if (!role) {
      caller.utils.createMessage(command.msg.channel.id, {
        embed: {
          color: caller.color.yellow,
          title: lang.titles.error,
          description: lang.commands.multi.ordinal.replace('$pos', caller.utils.ordinalSuffix(index + 1)),
        },
      });
      return;
    }
  ...
  try {
    ...
    caller.logger.warn(`[Multi] ${command.msg.channel.id} ${e.code} ${e.message.replace(/\n\s/g, '')}`);
    switch (e.code) {
      case 10003:
  ...
  let description = '';
  description += lang.commands.multi.set.replace('$roles', name).replace('$emoji', emoji);
  if (description) {
    description += `\n${lang.footer}`;
    caller.utils.createMessage(command.msg.channel.id, {
      embed: {
